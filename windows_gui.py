"""
windows_gui- 图形界面

Author:肖虎
Date:2022/6/4
"""

import PySimpleGUI as sg

from Report import pop_up
from main import main

default_meter_xlsx = 'resources/项目信息（煤矸石）.xlsx'
default_meter_docx = 'resources/电能表检验报告（邮件模板）.docx'
default_CT_xlsx = '.....'
default_CT_docxs = '..;.;..'
default_PT_xlsx = '......'
default_PT_docx = '........'
default_results_path = 'results'
ls = ['电能表报告', '电流互感器报告', '电压互感器报告']
icon = b''

sg.theme('LightGray')
layout = [
    [sg.Text('')],
    [sg.Text('选择项目信息（.xlsx文件）')],
    [sg.FileBrowse('选择一个文件', size=12, target='-XLSX-', file_types=(('Excel文件', '.xlsx'),)),
     sg.In(default_meter_xlsx, key='-XLSX-')],

    [sg.Text('')],
    [sg.Text('选择报告模板（.docx文件）')],
    [sg.FileBrowse('选择一个文件', size=12, target='-DOCX-', key='-DOCX_FB-', file_types=(('Word文件', '.docx'),)),
     sg.In(default_meter_docx, key='-DOCX-', disabled_readonly_background_color='LightGray')],

    [sg.Text('')],
    [sg.Text('选择存放文件的文件夹')],
    [sg.FolderBrowse('选择一个文件夹', size=12, target='-PATH-'),
     sg.In(default_results_path, key='-PATH-')],

    [sg.Text('')],
    [sg.Text('选择生成模式')],
    # 单选按钮 group_id=1
    [sg.R(ls[0], group_id=1, default=True, enable_events=True, key='-Radio11-'),
     sg.R(ls[1], group_id=1, enable_events=True, key='-Radio12-'),
     sg.R(ls[2], group_id=1, enable_events=True, key='-Radio13-')],

    # 编辑电流互感器选项
    [sg.Frame('CT选项', title_color='DarkGray', key='-CT_menu-', relief=sg.RELIEF_SUNKEN, visible=False, layout=[
        [sg.Text('选择电流互感器报告模板（一个或多个）')],
        [sg.FilesBrowse('选择文件', size=11, target='-DOCXS-', file_types=(('Word文件', '.docx'),)),
         sg.In(default_CT_docxs, key='-DOCXS-')]
    ])],

    [sg.Text('')],
    [sg.Button('确认生成')]
]

window = sg.Window('报告成生成器', layout, font=10, icon=icon)
button_color = window['-DOCX_FB-'].ButtonColor

while True:
    event, value = window.read()
    if event is None:
        break

    if event == '-Radio12-':
        window['-CT_menu-'].update(visible=True)
        window['-XLSX-'].update(default_CT_xlsx)
        window['-DOCX-'].update('', disabled=True)
        window['-DOCX_FB-'].update(button_color='LightGray', disabled=True)
    if event == '-Radio11-':
        window['-CT_menu-'].update(visible=False)
        window['-XLSX-'].update(default_meter_xlsx)
        window['-DOCX-'].update(default_meter_docx, disabled=False)
        window['-DOCX_FB-'].update(button_color=button_color, disabled=False)
    if event == '-Radio13-':
        window['-CT_menu-'].update(visible=False)
        window['-XLSX-'].update(default_PT_xlsx)
        window['-DOCX-'].update(default_PT_docx, disabled=False)
        window['-DOCX_FB-'].update(button_color=button_color, disabled=False)

    if event == '确认生成':
        xlsx_name = value['-XLSX-']
        docx_name = value['-DOCX-']
        results_path = value['-PATH-']
        docx_names = value['-DOCXS-']
        if value['-Radio11-']:
            model = '电能表'
        elif value['-Radio12-']:
            sg.Popup('还没研究呢，等着……', icon=icon)
            model = 'CT'
            docx_name = docx_names
        else:
            sg.Popup('说完了还乱点？！', icon=icon)
            model = 'PT'

        error_dict = main(xlsx_name, docx_name, results_path, model)
        pop_up(error_dict, results_path, icon)
window.close()
